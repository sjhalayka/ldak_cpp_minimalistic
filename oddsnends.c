...
if(fraction>=cumsum[try_]&&fraction<cumsum[try_+1]){break;}

count++;
if(count>100){printf("Error, can not find %f out of %f with %d - last try_ %d boundaries %f %f\n\n", fraction, cumsum[length], length, try_+1, cumsum[try_], cumsum[try_+1]);exit(1);}
}

if(flag==1)	//move down values
{
value=cumsum[try_+1]-cumsum[try_];
for(j=try_;j<length;j++){cumsum[j+1]-=value;}
}
