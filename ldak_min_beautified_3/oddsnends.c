...
int countrows(char *filename) {
  ...
  while (fscanf(input, "%c", &readchar) == 1) {
    if (readchar == 10) // new row, so add one
    {
      ...
      if (readchar ==
          10) // have two new lines in a row - only allowed if at end of file
      {
...
int countrows_plus(char *filename, int ncols) {
  ...
  while (fscanf(input, "%c", &readchar) == 1) {
    ...
    {
      if (readchar ==
          10) // new line starts with a newline - only allowed if end of file
      {
        if (fscanf(input, "%c", &readchar3) != 1) {
          break;
        }
        printf("Error, Row %d of %s appears to be empty\n\n", count + 1,
               filename);
    ...
    {
      if (count2 != ncols) {
        printf("Error reading %s, Row %d appears to have %d columns (whereas "
               "Row 1 has %d columns)\n\n",
               filename, count + 1, count2, ncols);
        exit(1);
      }
      count++;
    }
...
int countels(char *filename) {
  ...
  readchar2 = readchar;
  while (fscanf(input, "%c", &readchar) == 1) {
    if ((readchar2 == 9 || readchar2 == 10 || readchar2 == 32) &&
        readchar != 9 && readchar != 10 && readchar != 32) {
      count++;
    }
    readchar2 = readchar;
  }
...
int checkcols(char *filename, int ncols) {
  ...
  while (fscanf(input, "%c", &readchar) == 1) {
    if (readchar == 10) {
      printf("Error reading %s, Row %d appears to start with an empty line\n\n",
             filename, count + 1);
      exit(1);
    }
    if (readchar == 9 || readchar == 32) {
    ...
    if (count2 != ncols) {
      printf("Error reading %s, Row %d appears to have %d columns (whereas Row "
             "1 has %d columns)\n\n",
             filename, count + 1, count2, ncols);
      exit(1);
    }
...
void read_strings(char *strfile, char **str, int length, int *indexer, int col,
  ...
  found = 0;
  for (j = 0;j < count;
       j++) // read in first col-1 columns, then string, then skip to end
  {
    want = j;
    if (indexer != NULL) {
      want = indexer[found];
    ...
    if (j == want) // using row
    {
      if (strcmp(rs, "NA") == 0) {
        ...
        if (sscanf(rs, "%lf%c", values + found, &readchar) != 1) {
          printf("Error reading %s  Element %d of Row %d does not appear to be "
                 "numeric (%s)\n\n",
                 valfile, col, head + j + 1, rs);
          exit(1);
        }
      }
      found++;
...
void read_long(char *valfile, size_t *values, int length, int *indexer, int col,
  ...
  found = 0;
  for (j = 0;j < count;
       j++) // read in first col-1 columns, then value, then skip to end
  {
    want = j;
    if (indexer != NULL) {
      want = indexer[found];
    ...
    if (fscanf(input, "%s%c", rs, &readchar) != 2) {
      ...
      if (strcmp(rs, "NA") != 0) {
        printf(
            "Error reading %s  Element %d of Row %d is unrecognisable (%s)\n\n",
            valfile, col, head + j + 1, rs);
        exit(1);
      }
    }
...
void read_ids(char *idsfile, char **ids1, char **ids2, char **ids3, int length,
  ...
  if (count == 0) {
    printf("Error, %s contains no samples\n\n", idsfile);
    exit(1);
  }
  if (indexer != NULL) {
    if (indexer[length - 1] >= count) {
      printf("Error reading %s  has %d rows but should have at least %d\n\n",
             idsfile, count + head, head + indexer[length - 1] + 1);
      exit(1);
    }
  }
  if (countcols(idsfile) < shift + 2) {
    printf("Error reading %s  has %d columns but should have at least %d\n\n",
           idsfile, countcols(idsfile), shift + 2);
    exit(1);
  }
  ...
  found = 0;
  for (i = 0;i < count;i++) // read skip shift elements, read two elements,;
                              // then skip to end of line
  {
    ...
    for (j = 0;j < shift;j++) {;
      if (fscanf(input, "%s ", rs1) != 1) {
        printf("Error reading Element %d from Row %d of %s\n", j + 1,
               head + i + 1, idsfile);
        exit(1);
      }
    }
    if (fscanf(input, "%s %s%c", rs1, rs2, &readchar) != 3) {
      printf("Error reading Elements %d and %d from Row %d of %s\n", shift + 1,
             shift + 2, head + i + 1, idsfile);
      exit(1);
    }
    if (i == want) {
      if (ids1 != NULL) {
        copy_string(ids1, found, rs1);
      }
      if (ids2 != NULL) {
        copy_string(ids2, found, rs2);
      }
      if (ids3 != NULL) {
...
void read_ids_bgen(char *bgenfile, char **ids1, char **ids2, char **ids3,
  ...
  if (count2 != count) {
    printf("Error, %s seems to be corrupted (the numbers of samples are not "
           "consistent) - %d and %d\n\n",
           bgenfile, count, count2);
    exit(1);
  }
  ...
  for (i = 0;i < length;i++) {;
    ...
    for (mark = 0;mark < strlen(ids3temp[i]);mark++) {;
      if (ids3temp[i][mark] == '_') {
        break;
      }
    }
    if (mark == strlen(ids3temp[i])) {
      if (indexer != NULL) {
        printf("Error, the 'ID for Sample %d (%s) does not contain the "
               "character \"_\", so can not be separated\n\n",
    ...
    memcpy(ids2temp[i], ids3temp[i] + mark + 1,
           sizeof(char) * (strlen(ids3temp[i]) - mark - 1));
    ids2temp[i][strlen(ids3temp[i]) - mark - 1] = '\0';
    if (ids1 != NULL) {
      copy_string(ids1, i, ids1temp[i]);
    }
    if (ids2 != NULL) {
      copy_string(ids2, i, ids2temp[i]);
    }
    if (ids3 != NULL) {
      ids3[i] = malloc(sizeof(char) *
                       (strlen(ids1temp[i]) + strlen(ids2temp[i]) + 4));
  ...
  if (type == 1) {
    ...
    printf("(if the conversion has not worked, you should use \"--sample\" to "
           "provide the correct sample IDs)\n\n");
  }
  for (i = 0;i < length;i++) {;
    free(ids1temp[i]);
    free(ids2temp[i]);
...
void print_empty(char *readstring, const char *arg) {
  printf("Error, you can only use one main argument (you have used both %s and "
         "%s)\n\n",
         readstring, arg);
}
////////
void print_scaling(double power, int hwestand) {
  if (power == 0) {
    ...
    printf("Predictors will be centred then scaled by V^(%.4f/2) (option "
           "\"--power\"), ",
           power);
    if (hwestand == 0) {
      printf("where V is the observed variance of the predictor  to instead "
             "scale based on expected variance use \"--hwe-stand YES\"\n\n");
    } else {
      printf("where V=2*MAF*(1-MAF), the expected variance assuming "
             "Hardy-Weinberg Equilibrium  to instead scale based on observed "
             "variance use \"--hwe-stand NO\"\n\n");
    }
  }
}
////////
void print_qc(double minmaf, double maxmaf, double minvar, double minobs,
  ...
  if (count == 0) {
    if (genprobs < 2) {
      printf("To perform quality control of predictors use \"--min-maf\", "
             "\"--max-maf\", \"--min-var\" and/or \"--min-obs\"\n\n");
    } else {
      printf("To perform quality control of predictors use \"--min-maf\", "
             "\"--max-maf\", \"--min-var\", \"--min-obs\" and/or "
             "\"--min-info\"\n\n");
    }
  } else {
    ...
    count2 = 0;
    if (minmaf != -9999) {
      printf(" MAF < %.6f", minmaf);
      count2++;
    }
    if (maxmaf != -9999) {
      if (count2 > 0 && count2 < count - 1) {
        printf(",");
      }
      if (count2 > 0 && count2 == count - 1) {
        printf(" OR");
      }
      printf(" MAF > %.6f", maxmaf);
      count2++;
    }
    if (minvar != -9999) {
      if (count2 > 0 && count2 < count - 1) {
    ...
    if (mininfo != -9999) {
      ...
      printf(" Info < %.6f", mininfo);
    }
    printf("\n");
    if (num_files > 1) {
      printf("Note that this filtering will be performed after merging the "
             "datasets\n");
    }
    printf("\n");
  }
...
int append_check(char *filename, char *filename2, char *workdir) {
  ...
  {
    if (filename2[0] != '/') {
      sprintf(filename, "%s%s", workdir, filename2);
    } else {
      strcpy(filename, filename2);
    }
    sprintf(filename3, "%s/", filename);
    if (stat(filename3, &statstruct) == 0) {
      return (2);
    }
    // if(access(filename, R_OK)!=0){return(3);}
    if (stat(filename, &statstruct) != 0) {
...
///////////////////////////
int fill_names(char **datastems, char **bimstems, char **famstems, int k,
               char *udatafile, int dtype) {
  if (dtype == 1) {
...
// type=0 - keep quiet, type=1 - advise
{
  ...
  if ((strcmp(readstring, "FID") == 0 && strcmp(readstring2, "IID") == 0) ||
      (strcmp(readstring, "ID1") == 0 && strcmp(readstring2, "ID2") == 0)) {
    if (type == 1) {
      printf("%s has a header row\n", filename);
    }
    head = 1;
  }
  fclose(input);
...
// type=0 - keep quiet, type=1 - advise
{
  ...
  if (strcmp(readstring, findstring) == 0 ||
      strcmp(readstring, findstring2) == 0) {
    if (type == 1) {
      printf("%s has a header row\n", filename);
    }
    head = 1;
...
int find_head(char *findstring, char *filename, int length) {
  ...
  for (j = 0;j < length;j++) {;
    ...
    for (k = 0;k < j;k++) {;
      if (strcmp(readlist[k], readlist[j]) == 0) {
        printf(
            "Error reading %s  Columns %d and %d have the same name (%s)\n\n",
            filename, k + 1, j + 1, readlist[k]);
        exit(1);
      }
    }
  }
  ...
  count = -1;
  for (j = 0;j < length;j++) {;
    if (strcmp(findstring, readlist[j]) == 0) {
      count = j;
      break;
    }
  }
...
///////////////////////////
int reduce_values(double *values, int length, int *keeppreds_use, int *chr,
                  char **preds, double *cm, double *bp, double *cmbp,
                  char **along1, char **along2, char *al1, char *al2,
                  double *centres, double *mults, double *sqdevs, double *rates,
  ...
  for (j = 0;j < length;j++) {;
    if (values[j] > 0) {
      if (count != j) {
        ...
        free(preds[count]);
        copy_string(preds, count, preds[j]);
        if (cm != NULL) {
          cm[count] = cm[j];
        }
        if (bp != NULL) {
        ...
        if (along1 != NULL) {
          free(along1[count]);
          copy_string(along1, count, along1[j]);
        }
        if (along2 != NULL) {
          free(along2[count]);
          copy_string(along2, count, along2[j]);
        }
        al1[count] = al1[j];
        al2[count] = al2[j];
        if (centres != NULL) {
          centres[count] = centres[j];
        }
        if (mults != NULL) {
          mults[count] = mults[j];
        }
        if (sqdevs != NULL) {
          sqdevs[count] = sqdevs[j];
        }
        if (rates != NULL) {
          rates[count] = rates[j];
        }
  ...
  for (r = 0;r < num_regs;r++) {;
    ...
    for (j = 0;j < regindex[r][0];j++) {;
      if (usedpreds[regindex[r][1 + j]] !=
          -1) // still using this predictor, but it may have been moved
      {
        regindex[r][1 + count2] = usedpreds[regindex[r][1 + j]];
        count2++;
      }
    }
    if (count2 == 0) {
      printf("Error, all %d predictors in Region %d have zero weight\n\n",
             regindex[r][0], r + 1);
      exit(1);
    }
    regindex[r][0] = count2;
...
int try_flip(char *ch1a, char *ch2a, int a, char *ch1b, char *ch2b, int b) {
  ...
  if ((int)ch1a[a] + (int)ch2a[a] + (int)ch1b[b] + (int)ch2b[b] != 287) {
    return (0);
  }
  if (ch1a[a] != 'A' && ch1a[a] != 'C' && ch1a[a] != 'G' && ch1a[a] != 'T') {
    return (0);
  }
  if (ch2a[a] != 'A' && ch2a[a] != 'C' && ch2a[a] != 'G' && ch2a[a] != 'T') {
    return (0);
  }
  if (ch1b[b] != 'A' && ch1b[b] != 'C' && ch1b[b] != 'G' && ch1b[b] != 'T') {
    return (0);
  }
  if (ch2b[b] != 'A' && ch2b[b] != 'C' && ch2b[b] != 'G' && ch2b[b] != 'T') {
    return (0);
  }
  // check that ch1a and ch2a are not ambiguous (A+T or C+G), in which case, nor
  // are ch1b and ch2b
  if ((int)ch1a[a] + (int)ch2a[a] == 138 ||
      (int)ch1a[a] + (int)ch2a[a] == 149) {
    return (0);
  }
  // then flip ch1a and ch2a (the first pair)
  if (ch1a[a] == 'A' || ch1a[a] == 'T') {
    ch1a[a] = 149 - (int)ch1a[a];
  } else {
    ch1a[a] = 138 - (int)ch1a[a];
  }
  if (ch2a[a] == 'A' || ch2a[a] == 'T') {
    ch2a[a] = 149 - (int)ch2a[a];
  } else {
    ch2a[a] = 138 - (int)ch2a[a];
  }
  return (1);
}
////////
int check_comp(char **preds, char *al1, char *al2, int a, char **newnames,
               char *newal1, char *newal2, int b, int *chr, double *bp,
               int dcount) {
  // previously used to allow flipping
  // if(flip==1){try_flip(newal1,newal2,b,al1,al2,a);}
  if ((al1[a] != newal1[b] && al1[a] != newal2[b]) ||
      (al2[a] != newal1[b] && al2[a] != newal2[b])) {
    printf("Error, %s and %s have the same position (Chr%d:%.0f) but different "
           "alleles (%c %c and %c %c)\n",
           preds[a], newnames[b], chr[a], bp[a], al1[a], al2[a], newal1[b],
           newal2[b]);
    // if(try_flip(newal1,newal2,b,al1,al2,a)==1)
    //{printf("You can avoid this error by using  "--allow-flips YES " n");}
    printf("\n");
    exit(1);
  }
  if (strcmp(preds[a], newnames[b]) !=
      0) // names mismatch, use first unless only second rs
  {
    if (dcount < 10) {
      printf("Chr%d:%.0f is referred to as both %s and %s  ", chr[a], bp[a],
             preds[a], newnames[b]);
    }
    if ((preds[a][0] != 'r' || preds[a][1] != 's') &&
        (newnames[a][0] == 'r' && newnames[a][1] == 's')) {
      if (dcount < 10) {
        printf("will use latter\n");
        free(preds[a]);
        copy_string(preds, a, newnames[b]);
      }
    } else {
      if (dcount < 10) {
        printf("will use former\n");
      }
    }
    dcount = dcount + 1;
...
///////////////////////////
double compute_info(double *data, float *p0, float *p1, int length,
                    double missingvalue) {
  ...
  meand = sumd / indcount;
  meant = sumt / indcount;
  // now get var (data), expected (<data-meand, truth-meant>) and expected
  // var(truth)
  vard = 0;
  vart = 0;
  vardt = 0;
  for (i = 0;i < length;i++) {;
    if (data[i] != missingvalue) {
      ...
      vart += (1 - p0[i] - p1[i]) * pow(meant, 2) + p1[i] * pow(1 - meant, 2) +
              p0[i] * pow(2 - meant, 2);
...
///////////////////////////
int check_copy(char **str, int i, char *name, char *mvalue, char *id1,
               char *id2, char *filename, int type) {
  if (strcmp(name, mvalue) != 0) // suggesting something non-missing
  {
    if (strcmp(str[i], name) != 0 && strcmp(str[i], mvalue) != 0) {
      if (type == 1) {
        printf("Warning reading %s  the PID (%s) for Sample %s %s does not "
               "match that previously seen (%s)\nAll PIDs will be set to "
               "missing (0)\n\n",
               filename, name, id1, id2, str[i]);
      }
      if (type == 2) {
        printf("Warning reading %s  the MID (%s) for Sample %s %s does not "
               "match that previously seen (%s)\nAll MIDs will be set to "
               "missing (0)\n\n",
               filename, name, id1, id2, str[i]);
      }
      if (type == 3) {
        printf("Warning reading %s  the sex (%s) for Sample %s %s does not "
               "match that previously seen (%s)\nAll sexes will be set to "
               "missing (0)\n\n",
               filename, name, id1, id2, str[i]);
      }
      if (type == 4) {
        printf("Warning reading %s  the phenotype (%s) for Sample %s %s does "
               "not match that previously seen (%s)\nAll phenotypes will be "
               "set to missing (NA)\n\n",
               filename, name, id1, id2, str[i]);
      }
      return (1);
...
// flag=0 double, flag=1 float
{
  ...
  if (flag == 0) {
    ...
    size = (double)num_kins * ns / 1024 * ns / 1024 * 4 / 1024;
  }
  if (size > 1) {
    printf("Warning, to store kinships requires %.1f Gb", size);
    if (type == 1) {
      printf("  to reduce use \"--memory-save YES\"");
    }
    printf("\n\n");
  }
}
...
void data_warn2(int length1, int length2) {
  ...
  if (size > 1) {
    printf("Warning, to process the data requires %.1f Gb  if this is too "
           "high, you can reduce using \"--bit-size\" (currently %d)\n\n",
           size, length1);
  }
}
////////
void data_warn3(int length1, int length2) {
  ...
  if (size > 1) {
    printf("Warning, to process the data requires %.1f Gb  if this is too "
           "high, you can reduce using \"--bit-size\"\n\n",
           size);
  }
}
////////
...
// if flag=1, then must move down values
{
  ...
  if (fraction < 0 || fraction > cumsum[length]) {
    printf(
        ...
        fraction, cumsum[length]);
    exit(1);
  }
  if (cumsum[0] != 0) {
    printf("Error, first cumsum is not zero, please tell Doug\n\n");
    exit(1);
  }
  lower = 0;
  upper = length;
  count = 0;
  while (1) {
    try_ = lower + (upper - lower) / 2;
    if (fraction < cumsum[try_]) // move left
    {
      upper = try_;
      try_ = lower + (upper - lower) / 2;
    }
    if (fraction >= cumsum[try_ + 1]) // move right
    {
      lower = try_;
      try_ = lower + (upper - lower) / 2;
    }
    if (fraction >= cumsum[try_] && fraction < cumsum[try_ + 1]) {
      break;
    }
    count++;
    if (count > 100) {
      printf("Error, can not find %f out of %f with %d - last try_ %d "
             "boundaries %f %f\n\n",
             fraction, cumsum[length], length, try_ + 1, cumsum[try_],
             cumsum[try_ + 1]);
      exit(1);
    }
  }
  ...
  {
    value = cumsum[try_ + 1] - cumsum[try_];
    for (j = try_;j < length;j++) {;
      cumsum[j + 1] -= value;
    }
  }
  return (try_);
...
// convert numbers into indexes
{
  ...
  if (count <= 0) {
    printf(
        "Error reading the argument to \"--covar-numbers\" (%s)  this should "
        "contain numbers separated by commas or dashes (e.g., the argument "
        "\"1,2,4-6,8\" means retain Covariates 1, 2, 4, 5, 6 and 8)\n\n",
        covarnums);
    exit(1);
  }
  if (readint <= 0) {
    printf("Error, the argument to \"--covar-numbers\" (%s) includes %d (all "
           "numbers should be positive)\n\n",
           covarnums, readint);
    exit(1);
  }
  if (readint > max) {
    printf("Error, the argument to \"--covar-numbers\" (%s) includes %d, which "
           "is larger than the total number of covariates in %s (%d)\n\n",
           covarnums, readint, covarfile, max);
    exit(1);
  }
  indexer[readint - 1] = 1;
  while (count == 2) // more elements
  {
    if (readchar != ',' && readchar != '-') {
      printf("Error, the numbers in the argument to \"--covar-numbers\" (%s) "
             "should be separated by either commas or dashes (not %c)\n\n",
             covarnums, readchar);
      exit(1);
    }
    prevint = readint;
    ...
    if (readint > max) {
      printf(
          "Error, the argument to \"--covar-numbers\" (%s) includes %d, which "
          "is larger than the total number of covariates in %s (%d)\n\n",
          covarnums, readint, covarfile, max);
      exit(1);
    }
    ...
    {
      if (readint < prevint) {
        printf("Error, the argument to \"--covar-numbers\" (%s) contains the "
               "substring \"%d-%d\" (the number after a dash should not be "
               "lower than the number before the dash)\n\n",
               covarnums, prevint, readint);
        exit(1);
      }
      if (count == 2 && readchar == '-') {
        printf("Error, the argument to \"--covar-numbers\" (%s) contains the "
               "substring \"%d-%d\"- (at least one of these dashes should be a "
               "comma)\n\n",
               covarnums, prevint, readint);
        exit(1);
      }
      while (prevint < readint) {
        indexer[prevint]++;
        prevint++;
      }
    }
  ...
  for (j = 0;j < max;j++) {;
    if (indexer[j] > 1) {
      printf("Error, the argument to \"--covar-numbers\" (%s) specifies "
             "Covariate %d more than once\n\n",
             covarnums, j + 1);
      exit(1);
    }
  }
  ...
  printf("The argument to \"--covar-numbers\" (%s) specifies %d of the %d "
         "covariates in %s\n\n",
...
// convert names into indexes
{
  ...
  while (readchar != 10) {
    ...
    (void)fscanf(input, "%c", &readchar);
    if (readchar == ',') {
      printf("Error, one of the covariate names in %s contains a comma, so you "
             "can not use \"--covar-names\" (consider instead using "
             "\"--covar-numbers\")\n\n",
             covarfile);
      exit(1);
    }
  ...
  while (mark < strlen(covarnames)) {
    ...
    if (count < 2) {
      printf("Error, reading the argument to \"--covar-names\" (%s)  %s does "
             "not contain a covariate called %s\n\n",
             covarnames, covarfile, readstring);
      exit(1);
    }
    indexer[count - 2]++;
  }
  ...
  printf("The argument to \"--covar-names\" (%s) specifies %d of the %d "
         "covariates in %s\n\n",
