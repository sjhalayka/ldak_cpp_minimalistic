...
if (mode == 184) // work out min and max and print header columns
{
  if (nonsnp == 0) {
    ...
    minfloat = missingvalue;
    maxfloat = missingvalue;
    for (i = 0;i < num_samples_use;i++) {;
      value = data[(size_t)j * num_samples_use + i];
      if (value != missingvalue) {
        if (minfloat == missingvalue) {
          minfloat = value;
          maxfloat = value;
        }
        if (value < minfloat) {
...
{
  fprintf(output3, "%d %s %ld %c %c ", chr[bitstart + j], preds[bitstart + j],
          (long int)bp[bitstart + j], al1[bitstart + j], al2[bitstart + j]);
}
for (i = 0;i < num_samples_use;i++) {;
  value = data[(size_t)j * num_samples_use + i];
  if (mode == 181) {
    ...
    if (i % 4 == 3 || i == num_samples_use - 1) {
      fwrite(&onechar, sizeof(unsigned char), 1, output3);
    }
  }
  if (mode == 182) {
    if (value != missingvalue) {
      if (fabs(value - round(value)) < 0.00005) {
        fprintf(output3, "%d ", (int)round(value));
      } else {
        fprintf(output3, "%.4f ", value);
      }
    } else {
      fprintf(output3, "NA ");
    }
  ...
  if (mode == 185) {
    if (value == missingvalue) {
      fprintf(output3, "0.33 0.33 0.33 ");
    } else {
      value = ps[0][(size_t)j * num_samples_use + i];
      if (value == 0 || value == 1) {
        fprintf(output3, "%d ", (int)value);
      } else {
        fprintf(output3, "%.3f ", value);
      }
      ...
      value = 1 - ps[0][(size_t)j * num_samples_use + i] -
              ps[1][(size_t)j * num_samples_use + i];
      if (value == 0 || value == 1) {
        fprintf(output3, "%d ", (int)value);
      } else {
        fprintf(output3, "%.3f ", value);
