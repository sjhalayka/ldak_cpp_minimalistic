...
////////
int find_strings(char **stra, int lengtha, char **strb, int lengthb,
                 int *indexa, int *indexb, char *filenamea, char *filenameb,
                 int *predordera, int *predorderb, int type)
// sees which of stra are in strb - gets indexes of stra used and to which these
// match in strb if filenamea/filenameb provided, should check for duplicates
// type states which list needs sorting (type=3, must sort both)
// note that if duplicates in stra, then elements of strb can be found multiple
...
// should stop
{
  ...
  if (type == 1 || type == 3) // stra not sorted
  {
    if (predordera != NULL) // order provided
    {
      ...
      qsort(sptrsa, lengtha, sizeof(struct sorting_string),
            compare_sorting_string);
      for (a = 0;a < lengtha;a++) {;
        ordera[a] = sptrsa[a].index;
      }
      free(sptrsa);
    }
  ...
  if (type == 2 || type == 3) // strb not sorted
  {
    if (predorderb != NULL) // order provided
    {
      ...
      qsort(sptrsb, lengthb, sizeof(struct sorting_string),
            compare_sorting_string);
      for (b = 0;b < lengthb;b++) {;
        orderb[b] = sptrsb[b].index;
      }
      free(sptrsb);
    }
...
// type states which list needs sorting (type=3, must sort both)
{
  ...
  for (b = 0;b < nb;b++) {;
    indexb[b] = sptrsb[b].index;
  }
  counta = 0;
  countb = 0;
  if (strcmp(idsa[indexa[0]], idsb[indexb[0]]) < 0) // start with a
  {
    copy_string(allids, 0, idsa[indexa[0]]);
    counta = 1;
    found = 1;
  } else // start with b
  {
    copy_string(allids, 0, idsb[indexb[0]]);
    countb = 1;
    found = 1;
  }
  while (counta < na && countb < nb) {
    if (strcmp(idsa[indexa[counta]], idsb[indexb[countb]]) < 0) // a comes next
    {
      ...
      {
        copy_string(allids, found, idsa[indexa[counta]]);
        found++;
      }
      counta++;
    } else // try b
    {
      if (strcmp(allids[found - 1], idsb[indexb[countb]]) != 0) {
        copy_string(allids, found, idsb[indexb[countb]]);
        found++;
      }
      countb++;
    }
...
// type states which list needs sorting (type=3, must sort both)
{
  ...
  while (counta < na && countb < nb) {
    ...
    {
      if (strcmp(idsa[indexa[counta]], idsb[indexb[countb]]) < 0) {
        counta++;
      } else {
        countb++;
      }
...
///////////////////////////
int uni_preds(int *chra, double *bpa, char **predsa, char *al1a, char *al2a,
...
// a is existing, so always takes priority - can only change alleles in b
{
  ...
  if (chra[counta] < chrb[countb] ||
      (chra[counta] == chrb[countb] &&
       bpa[counta] <= bpb[countb])) // start with a
  {
    allchr[0] = chra[counta];
    allbp[0] = bpa[counta];
    copy_string(allnames, 0, predsa[counta]);
    allal1[0] = al1a[counta];
    allal2[0] = al2a[counta];
    counta++;
    found = 1;
  } else // start with b
  {
    allchr[0] = chrb[countb];
    allbp[0] = bpb[countb];
    copy_string(allnames, 0, predsb[countb]);
    allal1[0] = al1b[countb];
    allal2[0] = al2b[countb];
    countb++;
    found = 1;
  }
  while (counta < Na) {
  ...
  while (counta < Na && countb < Nb) {
    if (chra[counta] < chrb[countb] ||
        (chra[counta] == chrb[countb] &&
         bpa[counta] <= bpb[countb])) // a is next
    {
      // and must be new
      allchr[found] = chra[counta];
      allbp[found] = bpa[counta];
      copy_string(allnames, found, predsa[counta]);
      allal1[found] = al1a[counta];
      allal2[found] = al2a[counta];
      found++;
      counta++;
    } else // b next
    {
      if (chrb[countb] != allchr[found - 1] ||
          bpb[countb] != allbp[found - 1]) // and its new
      {
        allchr[found] = chrb[countb];
        allbp[found] = bpb[countb];
        copy_string(allnames, found, predsb[countb]);
        allal1[found] = al1b[countb];
        allal2[found] = al2b[countb];
        found++;
      } else // not new, check compatible
      {
        dcount += check_comp(allnames, allal1, allal2, found - 1, predsb, al1b,
                             al2b, countb, allchr, allbp, dcount);
      }
      countb++;
    }
    while (counta < Na) {
  ...
  if (dcount > 10) {
    printf("In total, %d predictors had different names\n", dcount);
  }
  if (dcount > 0) {
    printf("\n");
  }
...
////////
int inter_preds(int *chra, double *bpa, char **predsa, char *al1a, char *al2a,
                int Na, int *chrb, double *bpb, char **predsb, char *al1b,
                char *al2b, int Nb, int *allchr, double *allbp, char **allnames,
                char *allal1, char *allal2, int k) {
  int counta, countb, found, dcount;
  ...
  while (counta < Na && countb < Nb) {
    if (chra[counta] == chrb[countb] &&
        bpa[counta] == bpb[countb]) // add a then check b compatible
    {
      allchr[found] = chra[counta];
      allbp[found] = bpa[counta];
      copy_string(allnames, found, predsa[counta]);
      allal1[found] = al1a[counta];
      allal2[found] = al2a[counta];
      dcount += check_comp(allnames, allal1, allal2, found, predsb, al1b, al2b,
                           countb, allchr, allbp, dcount);
      counta++;
      countb++;
      found++;
    ...
    {
      if (chra[counta] < chrb[countb] ||
          (chra[counta] == chrb[countb] && bpa[counta] < bpb[countb])) {
        counta++;
      } else {
        countb++;
      }
