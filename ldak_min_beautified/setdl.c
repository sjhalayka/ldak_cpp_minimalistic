...
// set data_length to num_preds_use (number after filtering), then overwrite (or
// update) when necessary
data_length = num_preds_use;
keeppreds_use = malloc(sizeof(int) * num_preds);
for (j = 0; j < num_preds_use; j++) {
  keeppreds_use[j] = keeppreds[j];
}

////////

// now can reduce annotations and load cmbp (will contain either 1000000xcm or
// bp)
...
for (j = 0; j < data_length; j++) {
  ...
  if (window_cm != -9999) {
    cmbp[j] = 1000000 * allcm[keeppreds_use[j]];
  } else {
    cmbp[j] = allbp[keeppreds_use[j]];
  }
