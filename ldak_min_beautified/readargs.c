...
if (argc % 2 != 1) {
  printf("Error, there must be an even number of arguments (not %d); arguments "
         "should be provided in pairs\n\n",
         argc - 1);
  exit(1);
}
...
while (count < argc) {
  if (argv[count][0] != '-' || argv[count][1] != '-') {
    printf("Error, Argument %d is incorrect (%s); all odd arguments should "
           "begin with \"--\"\n\n",
           count, argv[count]);
    exit(1);
  }
  if (strcmp(argv[count + 1], "blank") == 0) {
    printf("Sorry, none of the even arguments can be the word \"blank\" (LDAK "
           "uses this to indicate empty files)\n\n");
    exit(1);
  }

  rs = malloc(sizeof(char) * (strlen(argv[count + 1]) + 1));
  for (j = 0; j < strlen(argv[count + 1]); j++) {
    rs[j] = toupper(argv[count + 1][j]);
  }
  rs[strlen(argv[count + 1])] = '\0';
  if (strcmp(rs, "NA") == 0 || strcmp(rs, "NAN") == 0) {
    printf("Error, none of the even arguments can be \"NA\" or \"NAN\", or "
           "lower-case versions of these (C uses these to indicate missing "
           "values)\n\n");
    exit(1);
  }
  free(rs);
...
if (argc > 1) {
  if (argc == 3) {
    printf("There is one pair of arguments:\n");
  } else {
    printf("There are %d pairs of arguments:\n", (argc - 1) / 2);
  }
  count = 1;
  while (count < argc) {
...
while (count < argc) {
  ...
  if (strcmp(argv[count], "--cut-weights") == 0) {
    if (mode != -9999) {
      print_empty(readstring, argv[count]);
      exit(1);
    }
    mode = 101;
    strcpy(folder2, argv[count + 1]);
    found = 1;
    strcpy(readstring, argv[count]);
  }
  ...
  if (strcmp(argv[count], "--family-quant") == 0) {
    printf(
        "Sorry, \"--family-quant\" has been replaced by \"--quant-her\"\n\n");
    exit(1);
  }

  if (strcmp(argv[count], "--family-binary") == 0) {
    printf(
        "Sorry, \"--family-binary\" has been replaced by \"--tetra-her\"\n\n");
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--kvik-step2") ==
      0) // will first assign as mode=134, then later change to 131 or 132
         // (print advice at bottom)
  {
    if (mode != -9999) {
      print_empty(readstring, argv[count]);
      exit(1);
    ...
    if (strcmp(prsfile2, "blank") != 0) {
      printf("Error, when using \"--kvik-step2\" you should not also use "
             "\"--PRS\" (\"--kvik-step2 %s\" is equivalent to using either "
             "\"--linear %s.step2\" or \"--logistic %s.step2\" with \"--PRS "
             "%s.step1\")\n\n",
             argv[count + 1], argv[count + 1], argv[count + 1],
             argv[count + 1]);
      exit(1);
    }

    mode = 134;
    sprintf(outfile2, "%s.step2", argv[count + 1]);
    sprintf(prsfile2, "%s.step1", argv[count + 1]);
    kvikstep = 2;
    found = 1;
    strcpy(readstring, argv[count]);
    strcpy(readstring2, argv[count + 1]);
  }

  if (strcmp(argv[count], "--GCTA-LOCO-step2") ==
      0) // will first assign as mode=134, then later change to 131 or 132
         // (print advice at bottom)
  {
    if (mode != -9999) {
    ...
    if (strcmp(prsfile2, "blank") != 0) {
      printf("Error, when using \"--GCTA-LOCO-step2\" you should not also use "
             "\"--PRS\" (\"--GCTA-LOCO-step2 %s\" is equivalent to using "
             "either \"--linear %s.step2\" or \"--logistic %s.step2\" with "
             "\"--PRS %s.step1\")\n\n",
             argv[count + 1], argv[count + 1], argv[count + 1],
             argv[count + 1]);
      exit(1);
  ...
  if (strcmp(argv[count], "--fastGWA-step2") ==
      0) // will first assign as mode=134, then later change to 131 or 132
         // (print advice at bottom)
  {
    if (mode != -9999) {
    ...
    if (strcmp(prsfile2, "blank") != 0) {
      printf("Error, when using \"--fastGWA-step2\" you should not also use "
             "\"--PRS\" (\"--fastGWA-step2 %s\" is equivalent to using either "
             "\"--linear %s.step2\" or \"--logistic %s.step2\" with \"--PRS "
             "%s.step1\")\n\n",
             argv[count + 1], argv[count + 1], argv[count + 1],
             argv[count + 1]);
  ...
  if (strcmp(argv[count], "--kvik-step3") == 0) {
    ...
    if (power != -9999) {
      printf("Error, when using \"--kvik-step3\" you should not also use "
             "\"--power\" (LDAK will use the scaling estimate from "
             "\"--kvik-step1\")\n\n");
      exit(1);
    }
    if (strcmp(pvafile2, "blank") != 0) {
      printf("Error, when using \"--kvik-step3\" you should not also use "
             "\"--pvalues\"  (LDAK will use the p-values produced by "
             "\"--kvik-step2\")\n\n");
      exit(1);
    }
    if (strcmp(sumsfile2, "blank") != 0) {
      printf("Error, when using \"--kvik-step3\" you should not also use "
             "\"--summary\" (LDAK will use the summary statistics produced by "
             "\"--kvik-step2\")\n\n");
      exit(1);
    }
    if (strcmp(respfile2, "blank") != 0) {
      printf("Error, when using \"--kvik-step3\" you should not also use "
             "\"--pheno\" (LDAK will perform the analysis using summary "
             "statistics instead of individual-level data)\n\n");
      exit(1);
    }
    if (amb != -9999) {
      printf("Error, when using \"--kvik-step3\" you should not also use "
             "\"--allow-ambiguous\" (LDAK will use an in-sample reference "
             "panel, so automatically sets \"--allow-ambiguous YES\")\n\n");
      exit(1);
    }

    mode = 140;
    sprintf(outfile2, "%s.step3", argv[count + 1]);
    sprintf(prsfile2, "%s.step1", argv[count + 1]);
    amb = 1;
    found = 1;
    strcpy(readstring, argv[count]);
  ...
  if (strcmp(argv[count], "--kvik-step1") == 0) {
    ...
    if (loco != -9999) {
      printf("Error, when using \"--kvik-step1\" you should not also use "
             "\"--LOCO\" (\"--kvik-step1 %s\" is equivalent to using "
             "\"--elastic %s.step1\" with \"--LOCO YES\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
    }

    mode = 154;
    sprintf(outfile2, "%s.step1", argv[count + 1]);
    loco = 1;
    kvikstep = 1;
    found = 1;
    strcpy(readstring, argv[count]);
  ...
  if (strcmp(argv[count], "--GCTA-LOCO-step1") == 0) {
    ...
    if (loco != -9999) {
      printf("Error, when using \"--GCTA-LOCO-step1\" you should not also use "
             "\"--LOCO\" (\"--GCTA-LOCO-step1 %s\" is equivalent to using "
             "\"--ridge %s.step1\" with \"--LOCO YES\", \"--power -1\", "
             "\"--check-pedigree NO\" and \"--binary NO\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
    }
    if (power != -9999) {
      printf("Error, when using \"--GCTA-LOCO-step1\" you should not also use "
             "\"--power\" (\"--GCTA-LOCO-step1 %s\" is equivalent to using "
             "\"--ridge %s.step1\" with \"--LOCO YES\", \"--power -1\", "
             "\"--check-pedigree NO\" and \"--binary NO\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
    ...
    if (checkped != -9999) {
      printf("Error, when using \"--GCTA-LOCO-step1\" you should not also use "
             "\"--check-pedigree\" (\"--GCTA-LOCO-step1 %s\" is equivalent to "
             "using \"--ridge %s.step1\" with \"--LOCO YES\", \"--power -1\", "
             "\"--check-pedigree NO\" and \"--binary NO\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
    }
    if (dichot != -9999) {
      printf("Error, when using \"--GCTA-LOCO-step1\" you should not also use "
             "\"--binary\" (\"--GCTA-LOCO-step1 %s\" is equivalent to using "
             "\"--ridge %s.step1\" with \"--LOCO YES\", \"--power -1\", "
             "\"--check-pedigree NO\" and \"--binary NO\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
  ...
  if (strcmp(argv[count], "--fastGWA-step1") == 0) {
    ...
    if (loco != -9999) {
      printf(
          "Error, when using \"--fastGWA-step1\" you should not also use "
          "\"--LOCO\" (\"--fastGWA-step1 %s\" is equivalent to using \"--ridge "
          "%s.step1\" with \"--LOCO YES\" and \"--fastGWA YES\")\n\n",
          argv[count + 1], argv[count + 1]);
      exit(1);
    }
    if (fastgwa != -9999) {
      printf(
          "Error, when using \"--fastGWA-step1\" you should not also use "
          "\"--fastGWA\" (\"--fastGWA-step1 %s\" is equivalent to using "
          "\"--ridge %s.step1\" with \"--LOCO YES\" and \"--fastGWA YES\")\n\n",
          argv[count + 1], argv[count + 1]);
      exit(1);
    }
  ...
  if (strcmp(argv[count], "--bfile") == 0) {
    if (dtype != -9999) {
      printf("Error, you can only use one from \"--bfile\", \"--bgen\", "
             "\"--sped\", \"--speed\", \"--gen\", \"--mbfile\", \"--msped\", "
             "\"--mspeed\" or \"--mgen\"\n\n");
      exit(1);
    }
  ...
  if (strcmp(argv[count], "--sp") == 0) {
    printf("Note that \"--sp %s\" is equivalent to using \"--gen %s.sp\", "
           "\"--bim %s.bim\" and \"--fam %s.fam\", with \"--gen-skip 0\", "
           "\"--gen-headers 0\" and \"--gen-probs 1\"\nIf the predictor file "
           "is gzipped, you should instead use \"--sp-gz\"\n\n",
    ...
    strcpy(unamefile2, argv[count + 1]);
    dtype = 11;
    binary = 0;
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--sp-gz") == 0) {
    printf("Note that \"--sp-gz %s\" is equivalent to using \"--gen "
           "%s.sp.gz\", \"--bim %s.bim\" and \"--fam %s.fam\", with "
           "\"--gen-headers 0\" and \"--gen-probs 1\"\n\n",
           argv[count + 1], argv[count + 1], argv[count + 1], argv[count + 1]);
    if (dtype != -9999) {
      printf("Error, you can only use one from \"--bfile\", \"--bgen\", "
  ...
  if (strcmp(argv[count], "--beagle-dose") == 0) {
    printf("Note that \"--beagle-dose\" is shorthand for using \"--gen\" with "
           "\"--gen-skip 1\", \"--gen-headers 3\" and \"--gen-probs 1\"\n\n");
    if (dtype != -9999) {
      printf("Error, you can only use one from \"--bfile\", \"--bgen\", "
             "\"--sped\", \"--speed\", \"--gen\", \"--mbfile\", \"--msped\", "
             "\"--mspeed\" or \"--mgen\"\n\n");
  ...
  if (strcmp(argv[count], "--beagle-probs") == 0) {
    printf("Note that \"--beagle-probs\" is shorthand for using \"--gen\" with "
           "\"--gen-skip 1\", \"--gen-headers 3\" and \"--gen-probs 3\"\n\n");
    if (dtype != -9999) {
      printf("Error, you can only use one from \"--bfile\", \"--bgen\", "
             "\"--sped\", \"--speed\", \"--gen\", \"--mbfile\", \"--msped\", "
             "\"--mspeed\" or \"--mgen\"\n\n");
  ...
  if (strcmp(argv[count], "--haps") == 0) {
    printf("Note that \"--haps\" is shorthand for using \"--gen\" with "
           "\"--gen-skip 0\", \"--gen-headers 5\" and \"--gen-probs 0\"\nIf %s "
           "does not contain predictor details, you should provide these using "
           "\"--bim\"\n\n",
           argv[count] + 1);
    if (dtype != -9999) {
      printf("Error, you can only use one from \"--bfile\", \"--bgen\", "
  ...
  if (strcmp(argv[count], "--bim") == 0) {
    strcpy(ubimfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--fam") == 0) {
    if (famhead != -9999) {
      printf("Error, you can only use one from \"--fam\", \"--sample\" and "
             "\"--fam-files\"\n\n");
      exit(1);
    }
  ...
  if (strcmp(argv[count], "--msp") == 0) {
    printf("Sorry, \"--msp\" is no longer valid; you should instead use "
           "\"--mgen\" with \"--gen-skip 0\", \"--gen-headers 0\", "
           "\"--gen-probs 1\" and \"--fam-files YES\"\n\n");
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--fam-files") == 0) {
    ...
    if (strcmp(argv[count + 1], "NO") == 0) {
      famhead = 2;
    }
    if (famhead == -1) {
      printf("Error, %s should be YES or NO (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--gen-skip") == 0) {
    ...
    if (genskip > 1) {
      printf("Warning, %s is typically 0 or 1 (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--gen-probs") == 0) {
    if (sscanf(argv[count + 1], "%d%c", &genprobs, &readchar) != 1) {
      printf("Error, %s should be 2, 3 or 4 (indicating the number of "
             "probabilities provided for each SNP), by 0 (if providing "
             "haplotypes) or by 1 (if providing dosages) (not %s)\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
    if (genprobs < 0 || genprobs > 4) {
      printf("Error, %s should be 2, 3 or 4 (indicating the number of "
             "probabilities provided for each SNP), by 0 (if providing "
             "haplotypes) or by 1 (if providing dosages) (not %s)\n\n",
             argv[count], argv[count + 1]);
  ...
  if (strcmp(argv[count], "--keep") == 0) {
    strcpy(bsampfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--remove") == 0) {
    strcpy(csampfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--extract") == 0) {
    strcpy(bpredfile2, argv[count + 1]);
    extract = 1;
    found = 1;
  }

  if (strcmp(argv[count], "--exclude") == 0) {
    strcpy(cpredfile2, argv[count + 1]);
    extract = 1;
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--chr") == 0) {
    onechr = -4;
    extract = 1;
    if (strcmp(argv[count + 1], "AUTO") == 0) {
      onechr = -1;
    }
    if (strcmp(argv[count + 1], "ODD") == 0) {
      onechr = -3;
    }
    if (strcmp(argv[count + 1], "EVEN") == 0) {
      onechr = -2;
    }
    if (onechr == -4) {
      if (sscanf(argv[count + 1], "%d%c", &onechr, &readchar) != 1) {
        printf("Error, %s should be a non-negative integer, AUTO (to use only "
               "Chr 1-22), ODD or EVEN (not %s)\n\n",
               argv[count], argv[count + 1]);
        exit(1);
      }
      if (onechr <= 0) {
  ...
  if (strcmp(argv[count], "--minmaf") == 0 ||
      strcmp(argv[count], "--maxmaf") == 0 ||
      strcmp(argv[count], "--minvar") == 0 ||
      strcmp(argv[count], "--minobs") == 0 ||
      strcmp(argv[count], "--mininfo") == 0) {
    printf("Sorry, \"--minmaf\", \"--maxmaf\", \"--minvar\", \"--minobs\" and "
           "\"--mininfo\" have been replaced by \"--min-maf\", \"--max-maf\", "
           "\"--min-var\", \"--min-obs\" and \"--min-info\"\n\n");
    exit(1);
  }

  if (strcmp(argv[count], "--maf") == 0) {
    printf("Error, %s is not a recognised argument (to filter based on MAF, "
           "use \"--min-maf\" and/or \"--max-maf\")\n\n",
           argv[count]);
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--geno") == 0) {
    printf("Error, %s is not a recognised argument (to filter based on "
           "callrate, use \"--min-obs\")\n\n",
           argv[count]);
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--predictor-means") == 0) {
    strcpy(centresfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--weights") == 0) {
    strcpy(weightsfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--power") == 0) {
    if (gctastep == 1) {
      printf("Error, when using \"--GCTA-LOCO-step1\" you should not also use "
             "\"--power\" (\"--GCTA-LOCO-step1 %s\" is equivalent to using "
             "\"--ridge %s.step1\" with \"--LOCO YES\", \"--power -1\" and "
             "\"--binary NO\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
    }
    ...
    if (power < -1.25 || power > 0.25) {
      printf("Warning, %s is typically within [-1.25,0.25] (not %s); when "
             "analyzing human data, we generally recommend -0.25\n\n",
             argv[count], argv[count + 1]);
    }
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--pvalues") == 0) {
    ...
    strcpy(pvafile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--importances") == 0) {
    strcpy(impfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--encoding") == 0) {
    encoding = -1;
    if (strcmp(argv[count + 1], "ADD") == 0) {
      encoding = 1;
    }
    if (strcmp(argv[count + 1], "DOM") == 0) {
      encoding = 2;
    }
    if (strcmp(argv[count + 1], "REC") == 0) {
      encoding = 3;
    }
    if (strcmp(argv[count + 1], "HET") == 0) {
      encoding = 4;
    }
    if (strcmp(argv[count + 1], "MINOR") == 0) {
      encoding = 5;
    }
    if (strcmp(argv[count + 1], "MISS") == 0) {
      encoding = 6;
    }
    if (encoding == -1) {
      printf(
          "Error, %s should be ADD, DOM, REC, HET, MINOR or MISS (not %s)\nThe "
          "additive model codes genotypes 0/1/2, indicating the number of A1 "
          "alleles; the dominant, recessive and heterozygous models instead "
          "code genotypes 0/2/2, 0/0/2 and 0/2/0, respectively: the missing "
          "model codes 1 if the genotype is missing, else 0; the minor allele "
          "model matches the additive model, except the counts always "
          "correspond to the least-observed allele\n\n",
          argv[count], argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--threshold") == 0) {
    ...
    if (threshold < 0.5 || threshold > 1) {
      printf("Error, %s should be a number within [0.5,1] (not %s); typical "
  ...
  if (strcmp(argv[count], "--min-prob") == 0) {
    if (sscanf(argv[count + 1], "%lf%c", &minprob, &readchar) != 1) {
      printf("Error, %s should be a number within [0.5,1), or by 0 if you want "
             "to sample genotypes based on probabilities (not %s); typical "
             "values are 0.9 or 0.95\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
    if (minprob != 0 && (minprob < 0.5 || minprob >= 1)) {
      printf("Error, %s should be a number within [0.5,1), or by 0 if you want "
             "to sample genotypes based on probabilities (not %s); typical "
             "values are 0.9 or 0.95\n\n",
             argv[count], argv[count + 1]);
  ...
  if (strcmp(argv[count], "--grm") == 0) {
    strcpy(kinname2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--mgrm") == 0) {
    strcpy(kinlist2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--pheno") == 0) {
    ...
    strcpy(respfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--mpheno") == 0) {
    if (strcmp(argv[count + 1], "-1") == 0) {
      printf("Sorry, to analyze all phenotypes, you should now use \"--mpheno "
             "ALL\" (instead of \"--mpheno -1\")\n\n");
      exit(1);
    }
    ...
    if (strcmp(argv[count + 1], "ALL") == 0) {
      ...
      if (sscanf(argv[count + 1], "%d%c", &mpheno, &readchar) != 1) {
        printf("Error, %s should be a non-negative integer or \"ALL\" (not "
               "%s)\n\n",
               argv[count], argv[count + 1]);
        exit(1);
      }
  ...
  if (strcmp(argv[count], "--summary") == 0) {
    ...
    strcpy(sumsfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--summary2") == 0) {
    strcpy(sums2file2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--qcovar") == 0) {
    printf("Error, %s is not a recognised argument (LDAK uses \"--covar\" for "
           "quantitative covariates, and \"--factors\" for categorical "
           "covariates)\n\n",
           argv[count]);
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--covar") == 0) {
    strcpy(covarfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--covar-numbers") == 0) {
    strcpy(covarnums, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--covar-names") == 0) {
    strcpy(covarnames, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--enviro") == 0) {
    strcpy(envfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--top-snps") == 0) {
    printf("Sorry, there was a typo in the supplement; instead of "
           "\"--top-snps\" you should use \"--top-preds\"\n\n");
    exit(1);
  }

  if (strcmp(argv[count], "--top-preds") == 0) {
    strcpy(topfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--factors") == 0) {
    strcpy(factorfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--covar-PRS") == 0) {
    strcpy(povarfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--offset") == 0) {
    strcpy(offsetfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--no-thin") == 0) {
    ...
    if (strcmp(argv[count + 1], "DONE") == 0) {
      nothin = 2;
    }
    if (nothin == -1) {
      printf("Error, %s should be YES, NO or DONE (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--window-length") == 0) {
    if (strcmp(argv[count + 1], "-1") == 0) {
      printf("Sorry, to to use the whole chromosome, you should now use "
             "\"--window-length ALL\" (instead of \"--window-length -1\")\n\n");
      exit(1);
    }
    ...
    if (strcmp(argv[count + 1], "ALL") == 0) {
      ...
      if (sscanf(argv[count + 1], "%d%c", &window_length, &readchar) != 1) {
        printf("Error, %s should be a non-negative integer, or \"ALL\" if you "
               "want to use the whole chromosome (not %s)\n\n",
               argv[count], argv[count + 1]);
        exit(1);
      }
  ...
  if (strcmp(argv[count], "--section-length") == 0) {
    ...
    if (section_length <= 0) {
      printf("Error, %s should a positive integer (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--section-buffer") == 0) {
    printf("Error, \"--section-buffer\" is no longer valid; you should instead "
           "use \"--buffer-kb\" or \"--buffer-length\" to specify the buffer "
           "in kb or as a fixed number of predictors\n\n");
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--infos") == 0) {
    printf("Sorry, it is no longer possible to include information scores when "
  ...
  if (strcmp(argv[count], "--max-time") == 0) {
    ...
    if (maxtime <= 0) {
      printf("Error, %s should be a positive number (not %s) providing the "
  ...
  if (strcmp(argv[count], "--targets") == 0) {
    strcpy(targetfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--male-dosage") == 0) {
    ...
    if (dosage < 0 || dosage > 1) {
      printf("Warning, %s is typically within [0,1] (not %s)\n\n", argv[count],
             argv[count + 1]);
    }
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--males") == 0) {
    strcpy(malesfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--partial") == 0) {
    ...
    if (partial == -1) {
      printf("Error, %s must be YES or NO (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--shortcut") == 0) {
    ...
    if (shortcut == -1) {
      printf("Error, %s must be NO (not %s)\n\n", argv[count], argv[count + 1]);
      exit(1);
    }
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--overlaps") == 0) {
    strcpy(oversfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--PRS") == 0) {
    ...
    if (mode == 140) {
      printf(
          "Error, when using \"--kvik-step3\" you should not also use "
          "\"--PRS\" (LDAK will use the PRS produced by \"--kvik-step1\")\n\n");
      exit(1);
    }

    strcpy(prsfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--spa-side") == 0) {
    spaside = -1;
    if (strcmp(argv[count + 1], "ONE") == 0) {
      spaside = 1;
    }
    if (strcmp(argv[count + 1], "TWO") == 0) {
      spaside = 2;
    }
    if (spaside == -1) {
      printf("Error, %s should be ONE or TWO (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--spa-threshold") == 0) {
    ...
    found = 1;
    if (spathresh <= 0 || spathresh > 1) {
      printf("Error, %s should be number within (0,1] (not %s)\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
  }
  ...
  if (strcmp(argv[count], "--spa-range") == 0) {
    ...
    if (spamax <= 0) {
      printf("Error, %s should be positive number (not %s)\n\n", argv[count],
             argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--duos") == 0) {
    ...
    if (strcmp(argv[count + 1], "FATHERS") == 0) {
      duos = 2;
    }
    if (strcmp(argv[count + 1], "MOTHERS") == 0) {
      duos = 3;
    }
    if (duos == -1) {
      printf("Error, %s should be ALL, FATHERS OR MOTHERS (not %s)\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
    found = 1;
  ...
  if (strcmp(argv[count], "--adjust-predictors") == 0) {
    ...
    if (strcmp(argv[count + 1], "PARTIAL") == 0) {
      adjpreds = 1;
    }
    if (adjpreds == -1) {
      printf("Error, %s should be YES or NO (not %s)\n\n", argv[count],
  ...
  if (strcmp(argv[count], "--sample-weights") == 0) {
    strcpy(sampwfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--transformation-inverse") == 0) {
    strcpy(transfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--genefile") == 0) {
    if (strcmp(genefile2, "blank") != 0 || chunks != -9999 ||
        chunksbp != -9999) {
      printf("Error, you can only use one from \"--genefile\", \"--chunks\" or "
             "\"--chunks-bp\"\n\n");
      exit(1);
    }
    strcpy(genefile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--her-limit") == 0) {
    if (sscanf(argv[count + 1], "%lf%c", &limit, &readchar) != 1) {
      printf("Error, %s should be zero (to turn off the limit) or a positive "
             "number (not %s)\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
  ...
  if (strcmp(argv[count], "--annotation-prefix") == 0) {
    strcpy(annpref2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--genic-predictors") == 0) {
    strcpy(genpreds2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--labels") == 0) {
    strcpy(labfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--regression-predictors") == 0) {
    strcpy(printfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--heritability-predictors") == 0) {
    strcpy(herfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--taglist") == 0) {
    strcpy(taglist2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--matlist") == 0) {
    strcpy(matlist2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--pathlist") == 0) {
    strcpy(pathlist2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--tagfile") == 0) {
    strcpy(tagfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--pathways") == 0) {
    strcpy(pathfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--alternative-tags") == 0) {
    strcpy(altfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--cv-predictors") == 0) {
    strcpy(cvsfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--categories") == 0) {
    strcpy(catfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--taus") == 0) {
    strcpy(taufile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--binary") == 0) {
    if (gctastep == 1) {
      printf("Error, when using \"--GCTA-LOCO-step1\" you should not also use "
             "\"--dichot\" (\"--GCTA-LOCO-step1 %s\" is equivalent to using "
             "\"--ridge %s.step1\" with \"--LOCO YES\", \"--power -1\" and "
             "\"--binary NO\")\n\n",
             argv[count + 1], argv[count + 1]);
      exit(1);
  ...
  if (strcmp(argv[count], "--break-points") == 0) {
    strcpy(blockfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--cors") == 0) {
    strcpy(corname2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--pseudos") == 0) {
    strcpy(pseudostem2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--model") == 0) {
    ptype = -1;
    if (strcmp(argv[count + 1], "mega") == 0) {
      ptype = 0;
    }
    if (strcmp(argv[count + 1], "lasso-sparse") == 0) {
      ptype = 1;
    }
    if (strcmp(argv[count + 1], "lasso") == 0) {
      ptype = 2;
    }
    if (strcmp(argv[count + 1], "ridge") == 0) {
      ptype = 3;
    }
    if (strcmp(argv[count + 1], "bolt") == 0) {
      ptype = 4;
    }
    if (strcmp(argv[count + 1], "bayesr") == 0) {
      ptype = 5;
    }
    if (strcmp(argv[count + 1], "bayesr-shrink") == 0) {
      ptype = 6;
    }
    if (strcmp(argv[count + 1], "elastic") == 0) {
      ptype = 7;
    }
    if (strcmp(argv[count + 1], "LDpred1") == 0) {
      ptype = 8;
    }
    if (strcmp(argv[count + 1], "LDpred2") == 0) {
      ptype = 9;
    }
    if (ptype == -1) {
      printf("Error, %s should be \"lasso-sparse\", \"lasso\", \"ridge\", "
             "\"bolt\", \"bayesr\", \"bayesr-shrink\", \"--elastic\" or "
             "\"--mega\" (not %s)\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
  ...
  if (strcmp(argv[count], "--jackknife-proportion") == 0) {
    ...
    if (jackprop < 0.01 || jackprop > 0.99) {
      printf("Error, %s should be a number within [0.01,0.99] (not %s)\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
  ...
  if (strcmp(argv[count], "--eigen") == 0) {
    strcpy(eigenfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--scorefile") == 0) {
    strcpy(scorefile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--coeffsfile") == 0) {
    strcpy(cofile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--final-effects") == 0) {
    strcpy(finalfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--num-causals") == 0) {
    if (strcmp(argv[count + 1], "-1") == 0) {
      printf("Sorry, if you want every predictor to be causal, you should now "
             "use \"--num-causals ALL\" (instead of \"--num-causals -1\")\n\n");
      exit(1);
    }
    ...
    if (strcmp(argv[count + 1], "ALL") == 0) {
      ...
      if (num_causals <= 0 && num_causals != -1) {
        printf("Error, %s should be a positive integer or \"ALL\" (not %s)\n\n",
               argv[count], argv[count + 1]);
        exit(1);
      }
  ...
  if (strcmp(argv[count], "--her") == 0) {
    ...
    if (her > 1) {
      printf("Error, %s should be a number within [0,1] (not %s); if you are "
             "sure you want a value greater than 1, use \"--her-big\"\n\n",
             argv[count], argv[count + 1]);
      exit(1);
    }
    if (her < 0) {
  ...
  if (strcmp(argv[count], "--probabilities") == 0) {
    strcpy(probsfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--causals") == 0) {
    strcpy(causalsfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--effects") == 0) {
    strcpy(effectsfile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--data-pairs") == 0) {
    strcpy(jackfile2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--profile") == 0) {
    strcpy(proffile2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--exclude-same") == 0) {
    printf("Sorry, \"--exclude-same\" has been replaced by "
           "\"--exclude-same-names\"\n\n");
    exit(1);
  }
  ...
  if (strcmp(argv[count], "--lista") == 0) {
    strcpy(predlista2, argv[count + 1]);
    found = 1;
  }

  if (strcmp(argv[count], "--listb") == 0) {
    strcpy(predlistb2, argv[count + 1]);
    found = 1;
  }
  ...
  if (strcmp(argv[count], "--num-blocks") == 0) {
    if (strcmp(argv[count + 1], "-1") == 0) {
      printf("Sorry, to do leave-one-sample-out jackknifing, you should now "
             "use \"--num-blocks ALL\" (instead of \"--num-blocks -1\")\n\n");
      exit(1);
    }
    ...
    if (strcmp(argv[count + 1], "ALL") == 0) {
      ...
      if (sscanf(argv[count + 1], "%d%c", &num_blocks, &readchar) != 1) {
        printf("Error, %s should be an integer greater than 2, or \"ALL\" if "
               "you want to do leave-one-sample-out jackknifing (not %s)\n\n",
               argv[count], argv[count + 1]);
        exit(1);
      }
      if (num_blocks <= 2 && num_blocks != -1) {
        printf("Error, %s should be an integer greater than 2, or \"ALL\" if "
               "you want to do leave-one-sample-out jackknifing (not %s)\n\n",
               argv[count], argv[count + 1]);
        exit(1);
  ...
  if (found == 0) {
    printf("Error, %s is not a recognised argument\n", argv[count]);
    exit(1);
  }
  count += 2;
} // end of while loop
...
if (kvikstep == 1) {
  printf("Note that \"--kvik-step1 %s\" is equivalent to using \"--elastic "
         "%s.step1\" with \"--LOCO YES\"\n\n",
         readstring2, readstring2);
}

if (gctastep == 1) {
  printf("Note that \"--GCTA-LOCO-step1 %s\" is equivalent to using \"--ridge "
         "%s.step1\" with \"--LOCO YES\", \"--power -1\", \"--check-pedigree "
         "NO\" and \"--binary NO\"\n\n",
         readstring2, readstring2);
}

if (faststep == 1) {
  printf("Note that \"--fastGWA-step1 %s\" is equivalent to using \"--ridge "
         "%s.step1\" with \"--LOCO YES\" and \"--fastGWA YES\"\n\n",
         readstring2, readstring2);
}
...
if (kvikstep == 2) {
  printf(
      "Note that \"--kvik-step2 %s\" is equivalent to using either \"--linear "
      "%s.step2\" or \"--logistic %s.step2\" with \"--PRS %s.step1\"\n\n",
      readstring2, readstring2, readstring2, readstring2);
}

if (gctastep == 2) {
  printf("Note that \"--GCTA-LOCO-step2 %s\" is equivalent to using either "
         "\"--linear %s.step2\" or \"--logistic %s.step2\" with \"--PRS "
         "%s.step1\"\n\n",
         readstring2, readstring2, readstring2, readstring2);
}
...
if (faststep == 2) {
  printf("Note that \"--fastGWA-step2 %s\" is equivalent to using either "
         "\"--linear %s.step2\" or \"--logistic %s.step2\" with \"--PRS "
         "%s.step1\"\n\n",
         readstring2, readstring2, readstring2, readstring2);
}
...
if (mode == 140) {
  printf(
      "Note that \"--kvik-step3 %s\" is equivalent to using \"--cut-genes "
      "%s.step3\" with \"--pvalues XXX\", followed by \"--calc-genes-reml "
      "%s.step3\" with \"--summary YYY\", \"--allow-ambiguous YES\" and "
      "\"--power ZZZ\", followed by \"--join-genes-reml %s.step3\", where XXX "
      "and YYY are p-values and summary statistics from \"--kvik-step2\", "
      "while ZZZ is the power estimate from \"--kvik-step1\"\n\n",
      readstring2, readstring2, readstring2, readstring2);
}
...
if (mode == -9999) {
  if (argc > 1) {
    printf("Error, each command requires a main argument; here is a list of "
           "main arguments\n\n");
  } else {
    printf("Each command requires a main argument; here is a list of main "
           "arguments\n\n");
  }
