  ...
  while (1) {
    ...
    if (cur_ind) // will use some samples from this block
    {
      ...
      do {
        ...
        if (new_shift ==
            16) // new_word is full, so must save and update / reset values
        {
    ...
    if (cur_ind) // will use some samples from this block
    {
      ...
      do {
        ...
        } else // new word is full, so must save and update / reset values
        {
          ...
          {
            new_word = (cur_word_rest >> (new_limit * 2)) &
                       ((1 << (2 * new_shift)) - 1);
...
void read_bed_fast(char *bedfile, double *data, double *centres, double *mults,
                   double *sqdevs, double *rates, double *infos,
                   int num_samples_use, int *keepsamps, int length,
                   int *keeppreds, int num_samples, int num_preds,
                   double missingvalue, int *bedzeros, int *bedones,
  ...
  if (flag == 1) // see whether strictly monotonic
  {
    ...
    if (flag == 4) // monotonic, so will subset using 16-sample indicators
    {
      ...
      for (i = 0; i < num_samples_use; i++) {
        keep16s[keepsamps[i] / 16] |= (1 << (keepsamps[i] % 16));
      }

      // decide whether to use sparse subset (flag=2) or block subset (flag=3)
      if (num_samples_use * 3 < num_samples * 2) {
        flag = 2;
      } else {
        flag = 3;
      }
  ...
  // set rowlengths (either numbers of char for each predictor or number rounded
  ...
  if (nread > keeppreds[length - 1] + 1 - keeppreds[0]) {
    nread = keeppreds[length - 1] + 1 - keeppreds[0];
  }

  // allocate (allchars and newchars are a bit longer than necessary, to ensure
  // no overflow when subsetting)
  ...
  if (ftello(input) != (off_t)sizeof(unsigned char) * rowlength * num_preds +
                           sizeof(unsigned char) * 3) {
    printf("Error reading %s; should have size %jd (%d samples x %d "
           "predictors), but instead has size %jd\n\n",
           bedfile,
           (off_t)sizeof(unsigned char) * rowlength * num_preds +
               sizeof(unsigned char) * 3,
           num_samples, num_preds, ftello(input));
    exit(1);
  }
  ...
  if (startchars[2] != 1) {
    printf("Error reading %s; can only read binary PLINK files stored in "
           "SNP-major mode\n\n",
           bedfile);
    exit(1);
  }
  ...
  for (j = 0; j < length; j++) {
    if (keeppreds[j] >= last) // read in new block of predictors
    {
      first = keeppreds[j];
      last = first + nread;
      if (last > keeppreds[length - 1] + 1) {
        last = keeppreds[length - 1] + 1;
      }

      if (fseeko(input,
                 (off_t)sizeof(unsigned char) * rowlength * first +
                     sizeof(unsigned char) * 3,
                 SEEK_SET) != 0) {
      ...
      if (fread(allchars, sizeof(unsigned char), (last - first) * rowlength,
                input) != (last - first) * rowlength) {
        printf("Error reading values for Predictors %d to %d from %s\n\n",
               first + 1, last, bedfile);
        exit(1);
      }
    }
    ...
    {
      subset_bits_sparse((int *)(allchars + mark * rowlength), (int *)newchars,
                         keep16s, num_samples_use);
    }
    if (flag == 3) // block subset
    {
      subset_bits_block((int *)(allchars + mark * rowlength), (int *)newchars,
                        keep16s, num_samples_use);
    }
    ...
    if (type != 0) // need to get stats
    {
      ...
      c2 = 0;
      if (flag == 0) {
        rowchars = allchars + mark * rowlength;
      } else {
        rowchars = newchars;
      }
      ...
      // now any remaining samples (will be at most three, so no need to update
      // rowchars)
      for (i2 = i; i2 < num_samples_use; i2++) {
      ...
      indcount = c0 + c1 + c2;
      if (indcount > 0) {
        mean = (double)(c1 + 2 * c2) / indcount;
        var = (double)(c1 + 4 * c2) / indcount - pow(mean, 2);
      } else {
        mean = 0;
        var = 0;
      }
      ...
      if (var > 0) {
        if (type == 1) {
          ...
          mults[j] = pow(var * indcount / num_samples_use, -0.5);
        }
      } else {
        mults[j] = -9999;
      }
...
void read_bed_full(char *bedfile, unsigned char **data_char,
                   int num_samples_use, int *keepsamps, int length,
                   int *keeppreds, int num_samples, int num_preds,
                   int maxthreads) {
  ...
  for (thread = 0; thread < maxthreads; thread++) {
    Mrowchars[thread] = malloc(sizeof(unsigned char) * rowlength);
  }
  ...
  if (ftello(Minput[0]) !=
    ...
    printf("Error reading %s; should have size %jd (%d samples x %d "
           ...
           (off_t)sizeof(unsigned char) * rowlength * num_preds +
               sizeof(unsigned char) * 3,
           num_samples, num_preds, ftello(Minput[0]));
    exit(1);
  }
  ...
  if (fread(startchars, sizeof(unsigned char), 3, Minput[0]) != 3)
    if (startchars[0] != 108 || startchars[1] != 27) {
      printf(
          "Error reading %s; does not appear to be in binary PLINK format\n\n",
          bedfile);
      exit(1);
    }
  if (startchars[2] != 1) {
...
// now can read
#pragma omp parallel for private(thread, threadstart, threadend, j, i, count,  \
                                 count2, twobits) schedule(static, 1)
  for (thread = 0; thread < maxthreads; thread++) {
    ...
    for (j = threadstart; j < threadend; j++) {
      ...
      if (keeppreds[j] != Mcurrent[thread]) {
        if (fseeko(Minput[thread],
                   (off_t)sizeof(unsigned char) * rowlength * keeppreds[j] +
                       sizeof(unsigned char) * 3,
                   SEEK_SET) != 0) {
      ...
      if (fread(Mrowchars[thread], sizeof(unsigned char), rowlength,
                Minput[thread]) != rowlength) {
        printf("Error reading values for Predictor %d from %s\n\n",
               keeppreds[j] + 1, bedfile);
        exit(1);
      }
      ...
      for (i = 0; i < num_samples_use; i++) {
        ...
        case 3:
          break;
        case 2:
          data_char[j][i / 4] += (1 << (2 * (i % 4)));
          break;
        case 0:
          data_char[j][i / 4] += (2 << (2 * (i % 4)));
          break;
        case 1:
          data_char[j][i / 4] += (3 << (2 * (i % 4)));
          break;
        }
      }
    } // end of j loop
...
int read_bed_fly(char *bedfile, double *data, int num_samples_use,
  ...
  const double bedbytes[32] = {2.0, 2.0,          missingvalue, 2.0,
                               ...
                               1.0, missingvalue, 0.0,          missingvalue,
                               2.0, 1.0,          missingvalue, 1.0,
                               1.0, 1.0,          0.0,          1.0,
...
void convert_probs(Bytef *uncompdata2, int num_samples, double *datap0,
  ...
  } else // must be 16 bits
  {
    for (i = 0; i < num_samples; i++) {
      ...
      datap1[i] =
          ((double)(*(unsigned short *)(uncompdata2 + 4 * i + 2))) / 65535;
    }
  }
}
...
int read_bgen_fly(char *bgenfile, double *data, float **probs,
  ...
  // for layout 1, can predict max compressed size (while uncompressed size is
  // fixed)
  ...
  if (count != num_preds) {
    printf("Error N3DK, please tell Doug %d %d\n\n", count, num_preds);
    exit(1);
  }
  ...
  if (count != num_samples) {
    printf("Error N3DL, please tell Doug %d %d\n\n", count, num_samples);
    exit(1);
  }
  ...
  if (bgen_comp == 2) {
    printf("Error N3DM, please tell Doug\n\n");
    exit(1);
  }
  ...
  for (j = start; j < end; j++) {
    ...
    if (bgen_layout ==
        1) // read 3n two-byte probabilities, and convert to dosages
    {
      ...
      } else // compressed
      {
        ...
        if (readint > (int)maxLen1) {
          printf(
              "Error, size of compressed probabilities for Predictor %d (%d) "
              "is larger than the maximum allowed (%d), please tell Doug\n\n",
              j + 1, readint, (int)maxLen1);
          exit(1);
        }

        if (fread(compdata1, 1, readint, input) != readint) {
          printf("Error reading compressed probabilities for Predictor %d from "
                 "%s\n\n",
                 keeppreds[j] + 1, bgenfile);
          exit(1);
        }
        ...
        if (uncompress(uncompdata1, &destLen, compdata1, sourceLen) != Z_OK) {
          printf(
              "Error decompressing probabilities for Predictor %d from %s\n\n",
              keeppreds[j] + 1, bgenfile);
          exit(1);
        }
      }
      ...
      for (i = 0; i < num_samples; i++) {
        ...
        if (prob0 + prob1 + prob2 == 0) {
          datatemp[i] = missingvalue;
          datap0[i] = 0;
          datap1[i] = 0;
        } else {
          value = prob0 + prob1 + prob2;
          datatemp[i] = (2 * prob0 + prob1) / value;
          datap0[i] = prob0 / value;
          datap1[i] = prob1 / value;
        }
      }
    } else // layout 2
    {
      if (bgen_comp ==
          0) // uncompressed - get size of uncompressed data, then read data
      {
      ...
      } else // compressed - get size of uncompressed and compressed data, then
             // read data
      {
        if (fread(&readint, 4, 1, input) != 1) {
          printf("Error reading storage size for Predictor %d from %s\n\n",
      ...
      // data starts n (4), nalleles (2), min ploid (1), max ploid (1), ploids
      // (n), flag (1), bits(1)
      ...
      if (ploid1 != 2 || ploid2 != 2) {
        if (wcount < 5) {
          printf("Warning, predictor %d is not haploid for all samples (the "
                 "ploidy ranges from %d to %d), so its values will be set to "
                 "missing\n\n",
        ...
        if (bits != 8 && bits != 16) {
          if (vcount < 5) {
            printf("Warning, predictor %d is coded using %d bits, so its "
                   "values will be set to missing (LDAK requires either 8 or "
                   "16 bits)\n\n",
          ...
          // correspond to genotypes A1A1 and A1A2 (phase=0) or A1 allele for
          // each haplotype (phase=1)

          count = 10 + num_samples + 2 * (int)bits / 8 * num_samples;
          if (readint2 != count) {
            printf("Error, uncompressed probabilities for predictor %d should "
                   "have size %d (not %d)\n\n",
                   keeppreds[j] + 1, count, readint2);
            exit(1);
          }

          // convert data to probabilities (those corresponding to missing are
          // redundant)
          convert_probs(uncompdata2 + 8 + num_samples + 2, num_samples, datap0,
                        datap1, bits);
          ...
          for (i = 0; i < num_samples; i++) {
            ...
            if (flag == 1) {
              ...
              datatemp[i] = 2 * datap0[i] + datap1[i];
            }
          }
        } // end of correct bits
      }   // end of haploid
    ...
    if (threshold != -9999) // convert dosage to hard call
    {
      for (i = 0; i < num_samples; i++) {
        value = missingvalue;
        if (datatemp[i] < 1 - threshold) {
          value = 0.0;
        }
        if (datatemp[i] > 1 + threshold) {
          value = 2.0;
        }
        if (datatemp[i] >= threshold && datatemp[i] <= 2 - threshold) {
          value = 1.0;
        }
        datatemp[i] = value;
      }
    ...
    if (minprob == 0) // sample genotypes from probabilities (overwrite dosages)
    {
      for (i = 0; i < num_samples; i++) {
        if (datatemp[i] != missingvalue) {
          ...
          datatemp[i] = 2.0;
          if (unifrand > datap0[i]) {
            datatemp[i] = 1.0;
          }
          if (unifrand > datap0[i] + datap1[i]) {
            datatemp[i] = 0.0;
          }
        }
      }
    ...
    if (minprob > 0) // select most likely genotype (minprob must be above 0.5,
                     // so only one of below is true)
    {
      for (i = 0; i < num_samples; i++) {
        if (datatemp[i] != missingvalue) {
          datatemp[i] = missingvalue;
          ...
          if (1 - datap0[i] - datap1[i] > minprob) {
            datatemp[i] = 0;
          }
        }
      }
    ...
    for (i = 0; i < num_samples_use; i++) {
      ...
      if (probs != NULL) {
        probs[0][(size_t)(j - start) * num_samples_use + i] =
            datap0[keepsamps[i]];
        probs[1][(size_t)(j - start) * num_samples_use + i] =
            datap1[keepsamps[i]];
      }
    }
  } // end of j loop
  if (wcount > 5) {
    printf("In total %d predictors are not haploid\n", wcount);
  }
  if (vcount > 5) {
    printf("In total %d predictors have unsuitable bit codings\n", vcount);
  }
...
int read_sped_fly(char *spedfile, double *data, int num_samples_use,
  ...
  if (ftello(input) != (off_t)sizeof(float) * num_samples * num_preds) {
    printf("Error reading %s; should have size %jd (%d samples x %d "
           "predictors), but instead has size %jd\n\n",
           spedfile, (off_t)sizeof(float) * num_samples * num_preds,
           num_samples, num_preds, ftello(input));
    exit(1);
  }
  fseeko(input, 0, SEEK_SET);
  ...
  for (j = start; j < end; j++) {
    if (keeppreds[j] != current) {
      if (fseeko(input, (off_t)sizeof(float) * num_samples * keeppreds[j],
                 SEEK_SET) != 0) {
        printf("Error reading %s; unable to find Predictor %d\n\n", spedfile,
               keeppreds[j] + 1);
        exit(1);
    ...
    {
      for (i = 0; i < num_samples; i++) {
        if (rowfloats[i] != missingvalue &&
            (rowfloats[i] < 0 || rowfloats[i] > 2)) {
          printf(
              "Error reading %s; the value (%.6f) for Predictor %d, Individual "
              "%d is outside [0,2]\nIf the data do not represent biallelic SNP "
              "allele counts you should use \"--SNP-data NO\"\n\n",
              spedfile, rowfloats[i], current + 1, i + 1);
          exit(1);
        }
      }
    }
    ...
    for (i = 0; i < num_samples_use; i++) {
      data[(size_t)(j - start) * num_samples_use + i] = rowfloats[keepsamps[i]];
    }
      ...
      for (i = 0; i < num_samples_use; i++) {
        ...
        if (value != missingvalue) {
          ...
          if (value <= 1 - threshold) {
            data[(size_t)(j - start) * num_samples_use + i] = 0.0;
          }
          if (value >= 1 + threshold) {
            data[(size_t)(j - start) * num_samples_use + i] = 2.0;
          }
          if (value >= threshold && value <= 2 - threshold) {
            data[(size_t)(j - start) * num_samples_use + i] = 1.0;
          }
        }
      }
    }
...
float read_speed_size(char *speedfile) {
  ...
  if (size != 1 && size != 2) {
    printf("Error reading %s; values should be stored using either 1 or 2 "
           "bytes (not %.1f)\n\n",
           speedfile, size);
    exit(1);
  }
...
void read_speed_full(char *speedfile, float *speedstarts, float *speedscales,
                     unsigned char **data_char, unsigned short **data_short,
                     int num_samples_use, int *keepsamps, int length,
  ...
  if (size == 1) {
    Mrowchars = malloc(sizeof(unsigned char *) * maxthreads);
    for (thread = 0; thread < maxthreads; thread++) {
      Mrowchars[thread] = malloc(sizeof(unsigned char) * num_samples);
    }
  } else {
    Mrowshorts = malloc(sizeof(unsigned short *) * maxthreads);
    for (thread = 0; thread < maxthreads; thread++) {
      Mrowshorts[thread] = malloc(sizeof(unsigned short) * num_samples);
    }
  }
  ...
  fseeko(Minput[0], 0, SEEK_END);
  if (ftello(Minput[0]) !=
      (sizeof(float) * 16 + size * num_samples) * num_preds) {
    printf("Error reading %s; should have size %jd (%d samples and %d "
           "predictors), but instead has size %jd\n\n",
           speedfile,
           (off_t)(sizeof(float) * 16 + size * num_samples) * num_preds,
           num_samples, num_preds, ftello(Minput[0]));
    exit(1);
  }
  fclose(Minput[0]);
...
// now can read
#pragma omp parallel for private(thread, threadstart, threadend, j, i,         \
                                 minfloat, maxfloat) schedule(static, 1)
  for (thread = 0; thread < maxthreads; thread++) {
    threadstart = thread * threadlength;
    threadend = (thread + 1) * threadlength;
    if (threadend > length) {
    ...
    for (j = threadstart; j < threadend; j++) {
      ...
      if (keeppreds[j] != Mcurrent[thread]) {
        if (fseeko(Minput[thread],
                   (off_t)(sizeof(float) * 16 + size * num_samples) *
                       keeppreds[j],
                   SEEK_SET) != 0) {
          printf("Error reading %s; unable to find Predictor %d\n\n", speedfile,
                 j + 1);
      ...
      if (fread(startfloats, sizeof(float), 16, Minput[thread]) != 16) {
        printf("Error reading headers of Predictor %d in %s\n\n",
               keeppreds[j] + 1, speedfile);
        exit(1);
      }
      if (startfloats[0] != size) {
        printf("Error reading %s; the first predictor used %d bytes per value, "
               "while Predictor %d uses %.1f\n\n",
               speedfile, (int)size, keeppreds[j] + 1, startfloats[0]);
        exit(1);
      }

      minfloat = startfloats[1];
      maxfloat = startfloats[2];
      if (nonsnp == 0 && (minfloat != 0 || maxfloat != 2)) {
        printf("Error, %s does not contain biallelic SNP allele counts; you "
               "should use \"--SNP-data NO\"\n\n",
               speedfile);
        exit(1);
      }
      ...
      if (size == 1) {
        ...
        speedscales[j] = (maxfloat - minfloat) / 65534;
      }

      if (size == 1) {
        if (fread(Mrowchars[thread], sizeof(unsigned char), num_samples,
                  Minput[thread]) != num_samples) {
          printf("Error reading values for Predictor %d from %s\n\n",
                 keeppreds[j] + 1, speedfile);
          exit(1);
        }
        for (i = 0; i < num_samples_use; i++) {
          data_char[j][i] = Mrowchars[thread][keepsamps[i]];
        }
      } else {
        if (fread(Mrowshorts[thread], sizeof(unsigned short), num_samples,
                  Minput[thread]) != num_samples) {
          printf("Error reading values for Predictor %d from %s\n\n",
                 keeppreds[j] + 1, speedfile);
          exit(1);
        ...
        for (i = 0; i < num_samples_use; i++) {
          data_short[j][i] = Mrowshorts[thread][keepsamps[i]];
        }
      }
...
int read_speed_fly(char *speedfile, double *data, int num_samples_use,
                   int *keepsamps, int start, int end, int *keeppreds,
                   int num_samples, int num_preds, double missingvalue,
                   double threshold, int nonsnp) {
  int i, j, current;
  ...
  size = read_speed_size(speedfile);
  if (size == 1) {
    rowchars = malloc(sizeof(unsigned char) * num_samples);
  } else {
    rowshorts = malloc(sizeof(unsigned short) * num_samples);
  }
  ...
  for (j = start; j < end; j++) {
    ...
    if (size == 1) {
      if (fread(rowchars, sizeof(unsigned char), num_samples, input) !=
          num_samples) {
        printf("Error reading values for Predictor %d from %s\n\n",
               keeppreds[j] + 1, speedfile);
        exit(1);
      ...
      if (fread(rowshorts, sizeof(unsigned short), num_samples, input) !=
          num_samples) {
        printf("Error reading values for Predictor %d from %s\n\n",
               keeppreds[j] + 1, speedfile);
        exit(1);
    ...
    for (i = 0; i < num_samples_use; i++) {
      if (size == 1) // predictors can have 256 values
      {
        ...
        if (onechar == 255) {
          ...
          data[(size_t)(j - start) * num_samples_use + i] =
              (double)onechar / 254 * (maxfloat - minfloat) + minfloat;
        }
      } else // each value is two bytes - can have 65536 values
      {
        ...
        if (oneshort == 65535) {
          ...
          data[(size_t)(j - start) * num_samples_use + i] =
              (double)oneshort / 65534 * (maxfloat - minfloat) + minfloat;
        }
      }
    }
...
int read_gen_fly(char *genfile, double *data, float **probs,
                 int num_samples_use, int *keepsamps, int start, int end,
                 int *keeppreds, gzFile inputgz, int current, int num_samples,
  ...
  while (1) {
    while (current < keeppreds[start + found]) // skip rows
    {
      ...
      if (strlen(gzbuffer) >= size - 1) {
        printf("Error reading %s; Row %d is longer (%d) than expected/allowed "
               "(%d)\nPlease tell Doug\n\n",
               genfile, genskip + current + 1, (int)strlen(gzbuffer), size);
        exit(1);
      }
      current++;
    }
    ...
    {
      for (i = 0; i < num_samples_use; i++) {
        data[(size_t)found * num_samples_use + i] =
            data[(size_t)(found - 1) * num_samples_use + i];
        if (probs != NULL) {
          probs[0][(size_t)found * num_samples_use + i] =
              probs[0][(size_t)(found - 1) * num_samples_use + i];
          probs[1][(size_t)found * num_samples_use + i] =
              probs[1][(size_t)(found - 1) * num_samples_use + i];
        }
      }
      found++;
    } else // so new predictor
    {
      ...
      for (i = 0; i < num_samples; i++) {
        if (genprobs == 0) // have haps
        {
          ...
          if (flag == 0) {
            printf("Error reading haplotypes for Predictor %d, Individual %d "
                   "in %s (%c%c%c%c)\n\n",
                   genskip + current + 1, i + 1, genfile, buffptr[0],
                   buffptr[1], buffptr[2], buffptr[3]);
            exit(1);
          }
          buffptr += 4;
        }
        ...
        {
          if ((buffptr[0] == '0' || buffptr[0] == '1' || buffptr[0] == '2') &&
              buffptr[1] == ' ') {
            datatemp[i] = (int)buffptr[0] - 48;
            buffptr += 2;
          } else {
            if (sscanf(buffptr, "%[0-9eE.+-] %n", readstring, &offset) == 1) {
              ...
              if (sscanf(buffptr, "%s %n", readstring, &offset) != 1) {
                printf("Error reading value for Predictor %d, Individual %d in "
                       "%s\n\n",
                       genskip + current + 1, i + 1, genfile);
                exit(1);
              }
              buffptr += offset;
              ...
              {
                printf("Error reading %s; unknown value (%s) for Predictor %d, "
                       "Individual %d\n",
                       genfile, readstring, genskip + current + 1, i + 1);
                exit(1);
              }
            }
          }

          if (nonsnp == 0 && datatemp[i] != missingvalue &&
              (datatemp[i] < 0 || datatemp[i] > 2)) {
            printf("Error reading %s; the value (%.6f) for Predictor %d, "
                   "Individual %d is outside [0,2]\nIf the data do not "
                   "represent biallelic SNP allele counts you should use "
                   "\"--SNP-data NO\"\n\n",
                   genfile, datatemp[i], current + 1, i + 1);
            exit(1);
          }
        }
        ...
        if (genprobs > 1) // have probabilities
        {
          ...
          if ((buffptr[0] == '0' || buffptr[0] == '1') && buffptr[1] == ' ') {
            prob0 = (int)buffptr[0] - 48;
            offset += 2;
            buffptr += 2;
          } else {
            prob0 = strtod(buffptr, &buffptr2);
            buffptr = buffptr2 + 1;
          }

          // read prob1
          if ((buffptr[0] == '0' || buffptr[0] == '1') && buffptr[1] == ' ') {
            prob1 = (int)buffptr[0] - 48;
            offset += 2;
            buffptr += 2;
          } else {
            prob1 = strtod(buffptr, &buffptr2);
            buffptr = buffptr2 + 1;
          }
          ...
          {
            if (prob0 + prob1 > 1.02) {
              printf("Error reading %s; the two probabilities for Predictor "
                     "%d, Individual %d (Columns %d and %d) sum to %.6f "
                     "(greater than 1)\n\n",
                     genfile, genskip + current + 1, i + 1,
                     genheaders + i * genprobs + 1,
                     genheaders + i * genprobs + 2, prob0 + prob1);
              exit(1);
            }
          ...
          if (genprobs >= 3) {
            // read prob2
            if ((buffptr[0] == '0' || buffptr[0] == '1') && buffptr[1] == ' ') {
              prob2 = (int)buffptr[0] - 48;
              offset += 2;
              buffptr += 2;
            } else {
              prob2 = strtod(buffptr, &buffptr2);
              buffptr = buffptr2 + 1;
            }
          }

          if (genprobs ==
              3) // check 3 probs not too high (will later warn if too low)
          {
            if (prob0 + prob1 + prob2 > 1.02) {
              printf("Error reading %s; the three probabilities for Predictor "
                     "%d, Individual %d (Columns %d to %d) sum to %.6f "
                     "(greater than 1)\n\n",
                     genfile, genskip + current + 1, i + 1,
                     genheaders + i * genprobs + 1,
                     genheaders + i * genprobs + 3, prob0 + prob1 + prob2);
              exit(1);
            }
            if (prob0 + prob1 + prob2 >=
                .98) // difference treated as rounding error
            {
              value = prob0 + prob1 + prob2;
              prob0 = prob0 / value;
              prob1 = prob1 / value;
          ...
          if (genprobs == 4) // check 4 probs not too high nor too low (will
                             // later warn if 3 probs too low)
          {
            // read prob3
            if ((buffptr[0] == '0' || buffptr[0] == '1') && buffptr[1] == ' ') {
              prob3 = (int)buffptr[0] - 48;
              offset += 2;
              buffptr += 2;
            } else {
              prob3 = strtod(buffptr, &buffptr2);
              buffptr = buffptr2 + 1;
            }

            if (prob0 + prob1 + prob2 + prob3 > 1.02) {
              printf("Error reading %s; the four probabilities for Predictor "
                     "%d, Individual %d (Columns%d to %d) sum to %.6f (greater "
                     "than 1)\n\n",
                     genfile, genskip + current + 1, i + 1,
                     genheaders + i * genprobs + 1,
                     genheaders + i * genprobs + 4,
                     prob0 + prob1 + prob2 + prob3);
              exit(1);
            }
            if (prob0 + prob1 + prob2 + prob3 < 0.98) {
              printf("Error reading %s; the four probabilities for Predictor "
                     "%d, Individual %d (Columns%d to %d) sum to %.6f (less "
                     "than 1)\n\n",
                     genfile, genskip + current + 1, i + 1,
                     genheaders + i * genprobs + 1,
                     genheaders + i * genprobs + 4,
          ...
          if (minprob == -9999) // get dosage genotype
          {
            if (proball < 0.98) {
              if (wcount < 5) {
                printf("Warning reading %s; state probabilities for Predictor "
                       "%d, Individual %d (Columns %d to %d) sum to only %.6f "
                       "(less than 0.98), so value will be treated as missing. "
                       "This is uncommon using imputed data\n",
                       genfile, genskip + current + 1, i + 1,
                       genheaders + i * genprobs + 1,
                       genheaders + (i + 1) * genprobs, proball);
              }
              datatemp[i] = missingvalue;
              wcount++;
            } else // so must be above 0.98 (in which case has been rounded to
                   // 1)
            {
          ...
          } else // sample genotypes based on probabilities or select most
                 // likely
          {
            if (proball < 0.98) {
              if (wcount < 5) {
                printf(
                    "Warning reading %s; state probabilities for Predictor %d, "
                    "Individual %d (Columns %d to %d) sum to only %.6f (less "
                    "than 0.98). This is uncommon using imputed data\n",
                    genfile, genskip + current + 1, i + 1,
                    genheaders + i * genprobs + 1,
                    genheaders + (i + 1) * genprobs, proball);
              }
            ...
            if (minprob == 0) // sample from probabilities
            {
              ...
              if (unifrand > prob0 + prob1) {
                datatemp[i] = 0.0;
              }
              if (unifrand > prob0 + prob1 + prob2) {
                datatemp[i] = missingvalue;
              }
            } else // select most likely (minprob must be above 0.5, so only one
                   // of below is true)
            {
              datatemp[i] = missingvalue;
              if (prob0 > minprob) {
  ...
  if (wcount > 5) {
    printf("In total %d sets of probabilities summed to less than 0.98\n",
           wcount);
  }
