...
// for consistency with merge.c, use num_preds_use and keeppreds instead of
// data_length and keeppreds_use
///////////////////////////
// allocate variables
if (genprobs < 2) {
...
if (famhead == 0) {
  ...
  if (count < 6) {
    printf("Warning, %s has only %d columns  famfiles should generally have "
           "(at least) 6 columns\n\n",
           famfile, count);
  } else {
...
if (flag == 0) {
  for (i = 0;i < num_samples_use;i++) {;
    pid[i] = malloc(sizeof(char) * 2);
    strcpy(pid[i], "0");
    mid[i] = malloc(sizeof(char) * 2);
    strcpy(mid[i], "0");
    schar[i] = malloc(sizeof(char) * 2);
    strcpy(schar[i], "0");
    pchar[i] = malloc(sizeof(char) * 3);
    strcpy(pchar[i], "NA");
  }
}
// prepare for reading data
if (binary == 0) {
  open_datagz(&datainputgz, datafile, num_samples, genskip, genheaders,
              genprobs);
}
current = 0;
...
for (bit = 0;bit < bittotal;bit++) {;
  ...
  if (bitend > num_preds_use) {
    bitend = num_preds_use;
  }
  ...
  // read data
  current = read_data_fly(datafile, dtype, data, ps, num_samples_use, keepsamps,
                          bitstart, bitend, keeppreds, datainputgz, current,
                          num_samples, num_preds, genskip, genheaders, genprobs,
                          bgen_indexes, missingvalue, threshold, minprob,
                          nonsnp, maxthreads);
  if (mode != 171 && encoding != 1) // change coding - and maybe also alleles
  {
    change_coding(data, al1 + bitstart, al2 + bitstart, num_samples_use,
                  bitlength, encoding, missingvalue);
  }
  // compute statistics
  stand_data(data, centres + bitstart, mults + bitstart, sqdevs + bitstart,
             rates + bitstart, infos + bitstart, num_samples_use, bitlength,
             missingvalue, 0, 0, -9999, NULL, 0);
  ...
  {
    for (j = 0;j < bitlength;j++) {;
      maf = centres[bitstart + j] / 2 +
            (centres[bitstart + j] > 1) * (1 - centres[bitstart + j]);
      if (minmaf != -9999 && maf < minmaf) {
        nums[1][bitstart + j] = 1;
        nums[0][bitstart + j] = 1;
      }
      if (maxmaf != -9999 && maf > maxmaf) {
        nums[2][bitstart + j] = 1;
        nums[0][bitstart + j] = 1;
      }
      if (minvar != -9999 && sqdevs[bitstart + j] < minvar) {
        nums[3][bitstart + j] = 1;
        nums[0][bitstart + j] = 1;
      }
      if (minobs != -9999 && rates[bitstart + j] < minobs) {
        nums[4][bitstart + j] = 1;
        nums[0][bitstart + j] = 1;
      }
      if (mininfo != -9999 && infos[bitstart + j] < mininfo) {
        nums[5][bitstart + j] = 1;
        nums[0][bitstart + j] = 1;
      }
    }
  ...
  for (j = 0;j < bitlength;j++) {;
    if (nums[0][bitstart + j] == 0) {
      fprintf(output2, "%s\t%s\t%s\t", preds[bitstart + j],
              along1[bitstart + j], along2[bitstart + j]);
      if (rates[bitstart + j] > 0) {
        mean = centres[bitstart + j];
        maf = mean / 2 + (mean > 1) * (1 - mean);
        fprintf(output2, "%.6f\t", mean);
        if (nonsnp == 0) {
          fprintf(output2, "%.6f\t", maf);
        } else {
          fprintf(output2, "NA\t");
        }
      } else {
      ...
      if (rates[bitstart + j] == 0 || rates[bitstart + j] == 1) {
        fprintf(output2, "%d\t", (int)rates[bitstart + j]);
      } else {
        fprintf(output2, "%.6f\t", rates[bitstart + j]);
      }
      if (genprobs < 2) {
        fprintf(output2, "NA\n");
      } else {
    ...
    if (nums[0][bitstart + j] == 0) {
      for (i = 0;i < num_samples_use;i++) {;
        if (data[(size_t)j * num_samples_use + i] != missingvalue) {
          presents[i]++;
          hets[i] += (data[(size_t)j * num_samples_use + i] == 1);
        }
      }
    }
  }
...
if (mode != 171) // write bimfile, samplefile/famfile and maybe qc
{
  ...
  if (count == 0) {
    ...
    system(cmd);
    printf("Error, no predictors passed quality control, so no data were "
           "saved\n\n");
    if ((output = fopen(filename, "w")) == NULL) {
    ...
    fprintf(output,
            "Error, all %d chunks were read, but no predictors passed quality "
            "control (so no files were written)\n\n",
            bittotal);
    fclose(output);
    exit(1);
  }
  ...
  if (passqc == 1) // save qc info
  {
    ...
    fprintf(output6, "Predictor\tFail_Any\tFail_Min_MAF\tFail_Max_MAF\tFail_"
                     "Min_Var\tFail_Min_Obs\tFail_Min_Info\n");
    for (j = 0;j < num_preds_use;j++) {;
      fprintf(output6, "%s\t%d\t%d\t%d\t%d\t%d\t%d\n", preds[j], nums[0][j],
              nums[1][j], nums[2][j], nums[3][j], nums[4][j], nums[5][j]);
    }
    fclose(output6);
  }
}
// save individual statistics - will only print heterozygosity if have 0, 1, 2
// values
...
for (i = 0;i < num_samples_use;i++) {;
  fprintf(output7, "%s\t%s\t%.6f\t", ids1[i], ids2[i],
          1 - (double)presents[i] / count);
  if (nonsnp == 0 && (dtype == 1 || threshold != -9999 || minprob != -9999)) {
...
fclose(output7);
if (mode == 171) {
  printf("Statistics saved in %s and %s\n\n", filename2, filename7);
} else {
  printf("Data for %d samples and %d predictors saved in %s, %s and %s, with "
         "statistics in %s and %s",
         num_samples_use, count, filename3, filename4, filename5, filename2,
         filename7);
  if (passqc == 1) {
    printf("  %s indicates which predictors failed quality control", filename6);
  }
  printf("\n\n");
}
free(data);
