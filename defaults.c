...
// Set random seeds, and a few parameters, such as those used by multiple modes 
// and/or required for parsefiles.c 
...
omp_set_dynamic(0)
// stops number of threads varying 
...
if (nonsnp == -9999) {
  ...
  if (dtype == 3 || dtype == 4 || (dtype == 5 && genprobs == 1)) {
    printf("Warning, predictors are assumed to be biallelic SNP allele counts "
           "(i.e., take values within [0,2]): if this is not the case, you "
